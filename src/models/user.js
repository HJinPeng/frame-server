...
const userField = ['id', 'account', 'realname', 'email', 'phone', 'profile_photo', 'sex', 'status']
...
const user = {
  ...
  async getUserInfoById(id) {
    const sql = `SELECT ${stringifySqlField(userField, false)} FROM user WHERE id = ${id}`
    let result = await query(sql)
    if (Array.isArray(result) && result.length > 0) {
      result = underline2LowerCamelCase(result[0])
    } else {
  ...
  async page(condition) {
    ...
    let records = await filterByPage('user', stringifySqlField(userField), condition)
    return {
      total,
