...
const role = {
  ...
  async existRoleCode(roleCode, exceptId) {
    const sql = `SELECT role_code FROM role WHERE role_code = '${roleCode}' AND deleted != 1 ${
  ...
  async existRoleName(roleName, exceptId) {
    const sql = `SELECT role_name FROM role WHERE role_name = '${roleName}' AND deleted != 1  ${
      exceptId ? 'AND id != ' + exceptId : ''
    }`
    console.log(sql)
    let result = await query(sql)
